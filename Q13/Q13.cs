...
namespace TargetClearCS
{
    internal class original
    {
        ...
        static bool CheckIfUserInputEvaluationIsATarget(List<int> Targets, List<string> UserInputInRPN, ref int Score)
        {
            ...
            if (found == 2)
            {
                FindBetweenTargets(pos1, pos2, Targets, ref Score);
            }

            return UserInputEvaluationIsATarget;
        }

        private static void FindBetweenTargets(int pos1, int pos2, List<int> targets, ref int score)
        {
            int num = pos2 - pos1;
            int count = 1;
            ...
            while (num != count)
            {
                if (targets[pos1 + 1] != -1)
                {
                    targets.RemoveAt(pos1 + 1);
                    score += 2;
                    count++;
                }
                else
                {
                    targets.RemoveAt(pos1 + 2);
                    score += 2;
                    count++;
                }
            }

            targets.RemoveAt(pos1);
            targets.RemoveAt(pos1);
